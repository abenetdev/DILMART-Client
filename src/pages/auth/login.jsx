...
function AuthLogin() {
  ...
  async function onSubmit(e) {
    ...
    if (data?.payload?.success) {
      ...
      const userId = data.payload.user?._id || data.payload.user?.id;
      if (userId) dispatch(mergeGuestCart(userId));
      toast({ title: "Welcome back!" });

      // Role-based redirect: honour ?redirect= only for regular users
      const role = data.payload.user?.role;
      let destination;
      if (role === "vendor") {
        ...
        // Regular user — honour the redirect param
        destination = redirectTo;
      }
      navigate(destination, { replace: true });
    } else if (data?.payload?.requiresVerification) {
      // Email not verified — send to OTP page
      const unverifiedEmail = data.payload.email || form.email;
      toast({ title: "Verify your email first" });
      navigate(`/auth/verify-otp?email=${encodeURIComponent(unverifiedEmail)}`);
    } else {
