...
export default function AdminReturnRequestsTab() {
  ...
  const { list, current, isLoading, isSubmitting, error } = useSelector((s) => s.adminReturn);
  ...
  // Action buttons config based on current status
  function getActions() {
    if (!current) return [];
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <Input
          ...
          onKeyDown={(e) => e.key === "Enter" && dispatch(fetchAllReturns({ status: statusFilter !== "all" ? statusFilter : undefined, search }))}
        ...
        <div className="rounded-xl border overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b">
              <tr>
                {["Request ID","Order","Customer","Vendor","Reason","Requested","Approved","Status","Date",""].map((h) => (
                  <th key={h} className="text-left px-3 py-3 font-semibold text-slate-700 text-xs whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {list.map((r) => (
                <tr key={r._id} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                  <td className="px-3 py-3 font-mono text-[11px]">{r._id.slice(-8).toUpperCase()}</td>
                  <td className="px-3 py-3 font-mono text-[11px]">ORD-{r.orderId?._id?.toString().slice(-8).toUpperCase() || "?"}</td>
                  ...
                  <td className="px-3 py-3 text-xs">{r.approvedAmount != null ? currencyFormatter(r.approvedAmount) : "—"}</td>
          ...
          {!current ? (
            ...
            <div className="space-y-4 mt-2">
              <div className="flex flex-wrap gap-3 items-center justify-between">
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Request ID</p>
                  <p className="font-mono font-semibold text-sm">{current._id?.slice(-8).toUpperCase()}</p>
                </div>
                <ReturnStatusBadge status={current.status} />
              </div>

              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                <div><p className="text-[10px] text-muted-foreground uppercase">Order</p><p className="font-mono font-semibold">ORD-{current.orderId?._id?.toString().slice(-8).toUpperCase()}</p></div>
                ...
                {current.refundedAmount > 0 && (
                  <div><p className="text-[10px] text-muted-foreground uppercase">Refunded</p><p className="font-semibold text-blue-700">{currencyFormatter(current.refundedAmount)}</p></div>
                )}
              </div>
              ...
              {current.vendorDecisionReason && (
                <div className="rounded bg-slate-50 border px-3 py-2 text-xs space-y-0.5">
                  <p className="font-semibold text-slate-700">Vendor Decision: <span className="capitalize">{current.vendorDecision}</span></p>
              ...
              {current.evidence?.length > 0 && (
                <div>
                  <p className="text-xs text-muted-foreground mb-2">Evidence ({current.evidence.length} file{current.evidence.length !== 1 ? "s" : ""})</p>
              ...
              {activeAction === "reject" && (
                <div className="space-y-2 rounded-lg border border-red-200 bg-red-50 p-3">
                  ...
                  <Textarea value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} rows={2} placeholder="Provide a clear reason for the customer…" />
              ...
              {activeAction === "refund" && (
                <div className="space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-3">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    />
                    <Button size="sm" variant="ghost" className="text-xs" onClick={() => setApprovedAmount(String(current.requestedAmount))}>
                      Full amount
                    </Button>
                  </div>
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input type="checkbox" checked={changeOfMind} onChange={(e) => setChangeOfMind(e.target.checked)} className="rounded" />
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="bg-blue-600 text-white hover:bg-blue-700" disabled={isSubmitting || !approvedAmount}
                      onClick={() => act(adminApproveRefund, { approvedAmount: parseFloat(approvedAmount), changeOfMind }, "Refund approved. Use 'Process Refund Now' to execute.")}>
                      Approve Refund
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => setActiveAction(null)}>Cancel</Button>
                  </div>
